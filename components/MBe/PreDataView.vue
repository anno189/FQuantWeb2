      ...
      <div class="col-12" v-if="rowsIndustry">
        ...
        <div class="q-pa-sm">
          ...
          <template v-slot:body-cell="props">
            <q-td
              :props="props"
              :class="(props.row.orate50 >0 & props.row.orate50 < 5)?'bg-red-1':(props.row.orate50 >=5 & props.row.orate50 < 9)?'bg-red-2':(props.row.orate50 >=9)?'bg-red-3':(props.row.orate50 < 0 & props.row.orate50 > -7 )?'bg-green-1':(props.row.orate50 <= -7)?'bg-green-3':''"
            >
              {{props.value}}
            </q-td>
          </template>
...
export default defineComponent({
  ...
  methods: {
    getServerMarketData: async function () {
      ...
      const response = await http.get('https://stock.1dian.site/h5/data/predatatop.json', params)
      ...
      const res1 = await http.get('https://stock.1dian.site/h5/data/open_data_amount.json', params)
  ...
  setup () {
    ...
    const columnsHigh = ref([
      ...
      { name: '金额', align: 'right', label: '金额', field: 'oamo', sortable: true },
    ...
    const columnsIndustry = ref([
      ...
      { name: '前10金额', align: 'right', label: '前10金额', field: 'oamo10', sortable: true},
      { name: '前10涨幅', align: 'right', label: '前10涨幅', field: 'orate10', sortable: true },
      { name: '前10数量', align: 'right', label: '前10数量', field: 'count10', sortable: true },
      { name: '前20金额', align: 'right', label: '前20金额', field: 'oamo20', sortable: true},
      { name: '前20涨幅', align: 'right', label: '前20涨幅', field: 'orate20', sortable: true },
      { name: '前20数量', align: 'right', label: '前20数量', field: 'count20', sortable: true },
      { name: '前50金额', align: 'right', label: '前50金额', field: 'oamo50', sortable: true},
      { name: '前50涨幅', align: 'right', label: '前50涨幅', field: 'orate50', sortable: true },
      { name: '前50数量', align: 'right', label: '前50数量', field: 'count50', sortable: true },
      { name: '总金额', align: 'right', label: '总金额', field: 'oamo', sortable: true},
      { name: '总涨幅', align: 'right', label: '总涨幅', field: 'orate', sortable: true },
