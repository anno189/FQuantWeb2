...
export default defineComponent({
  ...
  methods: {
    getServerMarketData: async function () {
      const res1 = await http.get('https://stock.1dian.site/h5/data/str60p20_echarts_lines.json', {})
      this.v1lineOption=res1.data

      const res2 = await http.get('https://stock.1dian.site/h5/data/str60p20_echarts_pie.json', {})
      this.v1pieOption=res2.data

      const res3 = await http.get('https://stock.1dian.site/h5/data/str60p20_position.json', {})
      
      this.rowsM = res3.data.data.position
      this.rowsN = res3.data.data.tradelog

      const res9 = await http.get('https://stock.1dian.site/h5/data/str60p20.html', {})
  ...
  setup () {
    ...
    const columnsM = ref([
      ...
      { name: '时间', align: 'center', label: '时间', field: 'date', sortable: true },
      { name: '买入价', align: 'center', label: '买入价', field: 'values', sortable: true },
      { name: '浮盈', align: 'center', label: '浮盈', field: 'networth', sortable: true },
    ]);

    const columnsN = ref([
      { name: '近5日交易记录', label: '近5日交易记录', field: 'tradelog', sortable: false},
    ]);
