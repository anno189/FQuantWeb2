        ...
        <div class="q-pa-sm">
          ...
          >
            <template v-slot:body-cell="props">
              <q-td
                :props="props"
                :class="(props.row.Today == '机会, 看多')?'bg-red-3':(props.row.Today == '机会, 震荡上行')?'bg-red-1':(props.row.Today == '机会, 高位整理')?'bg-red-2':(props.row.Today == '量能不足, 空, 看空')?'bg-green-3':(props.row.Today == '量能不足, 空, 震荡下行')?'bg-green-2':(props.row.Today == '量能不足, 空, 高位整理')?'bg-green-1':(props.row.Today == '等变盘, 看多')?'bg-red-1':(props.row.Today == '等变盘, 震荡上行')?'bg-red-1':''"
              >
...
export default defineComponent({
  ...
  methods: {
    getServerMarketData: async function () {
      ...
      const res14 = await http.get('https://stock.1dian.site/h5/data/con3updata.json', params)
      this.rowscon3updata = res14.data.data.con3

      const res1 = await http.get('https://stock.1dian.site/h5/data/conceptspcorrweight.json', params)
      this.conceptspcorrweight = res1.data

      const res2 = await http.get('https://stock.1dian.site/h5/data/conceptwrweight.json', params)
      this.conceptwrweight = res2.data

      const res3 = await http.get('https://stock.1dian.site/h5/data/conceptcorrweight.json', params)
