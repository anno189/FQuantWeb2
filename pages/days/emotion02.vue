...
export default defineComponent({
  ...
  methods: {
    getServerMarketData: async function () {
      ...
      const res1 = await http.get('https://stock.1dian.site/h5/data/stock_emotion_02.json', {})
      const store = useAlinksStore()
      const alinks = new Array()
      ...
      for (let j = 0; j < this.data.length; j++){
        ...
        for (let i = 0; i < colz.length; i++) {
          if ( colz[i] == 'name')
            ...
            columnsz[i] = { name: colz[i], align: 'left', label: '涨跌', field: colz[i], sortable: false}
          else if ( colz[i] == 'memos')
            columnsz[i] = { name: colz[i], align: 'left', label: '备注', field: colz[i], sortable: false}
          else if ( colz[i] == 'HSZ')
            columnsz[i] = { name: colz[i], align: 'left', label: '换手率', field: colz[i], sortable: false}
          ...
          else if ( colz[i] == 'industry')
            columnsz[i] = { name: colz[i], align: 'left', label: '行业', field: colz[i], sortable: false}
          else if ( colz[i] == 'tag')
            columnsz[i] = { name: colz[i], align: 'left', label: 'TAG', field: colz[i], sortable: false}
          else if ( colz[i] == 'R3')
            columnsz[i] = { name: colz[i], align: 'left', label: '3日涨幅', field: colz[i], sortable: false}
          else
